...
def create_advanced_portfolio_interface(tickers):
    ...
    with optimization_cols[0]:
        if st.button("Optimize for Maximum Sharpe Ratio"):
            optimal_sharpe = analyzer.optimize_portfolio(target='sharpe')
            
            if optimal_sharpe:
                st.success("Sharpe-Optimal Portfolio Found!")
                
                opt_col1, opt_col2, opt_col3 = st.columns(3)
                
                with opt_col1:
                    st.metric("Expected Return", f"{optimal_sharpe['expected_return']*100:.2f}%")
                
                with opt_col2:
                    st.metric("Volatility", f"{optimal_sharpe['volatility']*100:.2f}%")
                
                with opt_col3:
                    st.metric("Sharpe Ratio", f"{optimal_sharpe['sharpe_ratio']:.2f}")
                ...
                for i, ticker in enumerate(tickers):
                    weights_data.append({
                        'Ticker': ticker,
                        'Optimal Weight': f"{optimal_sharpe['weights'][i]*100:.1f}%"
                    })
                
                weights_df = pd.DataFrame(weights_data)
                st.dataframe(weights_df, use_container_width=True)
            else:
                st.error("Optimization failed. Insufficient data or constraints.")
    
    with optimization_cols[1]:
        if st.button("Optimize for Minimum Variance"):
            optimal_variance = analyzer.optimize_portfolio(target='variance')
            
            if optimal_variance:
                st.success("Minimum Variance Portfolio Found!")
                
                opt_col1, opt_col2, opt_col3 = st.columns(3)
                
                with opt_col1:
                    st.metric("Expected Return", f"{optimal_variance['expected_return']*100:.2f}%")
                
                with opt_col2:
                    st.metric("Volatility", f"{optimal_variance['volatility']*100:.2f}%")
                
                with opt_col3:
                    st.metric("Sharpe Ratio", f"{optimal_variance['sharpe_ratio']:.2f}")
                ...
                for i, ticker in enumerate(tickers):
                    weights_data.append({
                        'Ticker': ticker,
                        'Optimal Weight': f"{optimal_variance['weights'][i]*100:.1f}%"
                    })
    ...
    if len(tickers) >= 2:
        ...
        if st.button("Generate Efficient Frontier"):
            with st.spinner("Calculating efficient frontier..."):
                efficient_portfolios = analyzer.generate_efficient_frontier()
            ...
            if efficient_portfolios:
                ...
                for ticker, data in portfolio_data.items():
                    if ticker in analyzer.returns_df.columns:
                        asset_return = analyzer.returns_df[ticker].mean() * 252
                        asset_volatility = analyzer.returns_df[ticker].std() * np.sqrt(252)
                ...
                st.info("The efficient frontier shows the optimal risk-return combinations. Points on the frontier represent portfolios with the highest expected return for a given level of risk.")
            else:
                st.error("Unable to generate efficient frontier. Insufficient data or optimization failed.")
